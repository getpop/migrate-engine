...
use PoP\Engine\Facades\ModelInstanceFacade;
...
class Engine
{
    ...
    public function __construct()
    {
        ...
        EngineFactory::setInstance($this);
    }
    ...
    {
        $siteconfiguration = Settings\SiteConfigurationProcessorManagerFactory::getInstance()->getProcessor();
    ...
    public function sendEtagHeader()
    {
        ...
        if (HooksAPIFacade::getInstance()->applyFilters('\PoP\Engine\Engine:outputData:addEtagHeader', true)) {
            ...
            $commoncode = HooksAPIFacade::getInstance()->applyFilters('\PoP\Engine\Engine:etag_header:commoncode', $commoncode);

            $cmsenginehelpers = \PoP\Engine\HelperAPIFactory::getInstance();
            header("ETag: ".$cmsenginehelpers->hash($commoncode));
        }
    }
    ...
    public function getSiteMeta()
    {
        ...
        if (Utils::fetchingSite()) {
            ...
            $meta[GD_DATALOAD_PARAMS][GD_URLPARAM_VERSION] = \PoP\Engine\Facades\ApplicationInfoFacade::getInstance()->getVersion();
    ...
    private function combineIdsDatafields(&$ids_data_fields, $dataloader_class, $ids, $data_fields)
    {
        $ids_data_fields[$dataloader_class] = $ids_data_fields[$dataloader_class] ?? array();
        foreach ($ids as $id) {
            // Make sure to always add the 'id' data-field, since that's the key for the dbobject in the client database
            $ids_data_fields[$dataloader_class][$id] = $ids_data_fields[$dataloader_class][$id] ?? array('id');
            $ids_data_fields[$dataloader_class][$id] = array_unique(
                array_merge(
                    $ids_data_fields[$dataloader_class][$id],
                    $data_fields ?? array()
                )
            );
        }
    ...
    protected function validateCheckpoints($checkpoints)
    {
    ...
    public function getModuleData($root_module, $root_model_props, $root_props)
    {
        ...
        foreach ($module_fullpaths as $module_path) {
            ...
            $dataset_meta = $dbobjectidsarray = array();
            $executed = $dbobjectids = null;
            if ($load_data) {
                // ------------------------------------------
                // Action Executers
                // ------------------------------------------
                ...
                if ($load_data) {
                    ...
                    $dbobjectidsarray = is_array($dbobjectids) ? $dbobjectids : array($dbobjectids);
                    ...
                    $this->combineIdsDatafields($this->ids_data_fields, $dataloader_class, $dbobjectidsarray, $data_fields);
    ...
    public function getDatabases()
    {
        ...
        while (!empty($this->ids_data_fields)) {
            ...
            $dataloader_class = key($this->ids_data_fields);
            $dataloader_ids_data_fields = $this->ids_data_fields[$dataloader_class];
            ...
            unset($this->ids_data_fields[$dataloader_class]);
            ...
            $already_loaded_ids_data_fields[$dataloader_class] = array_merge_recursive(
                $already_loaded_ids_data_fields[$dataloader_class],
                                        ...
                                        if ($field_ids) {
                                            foreach ($field_ids as $field_id) {
                                                ...
                                                if ($id_subcomponent_data_fields) {
                                                    $this->combineIdsDatafields($this->ids_data_fields, $subcomponent_dataloader_class, array($field_id), $id_subcomponent_data_fields);
                                                    $this->initializeDataloaderEntry($this->dbdata, $subcomponent_dataloader_class, $module_path_key/*$subcomponent_module_path_key*/);
                                                    $this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]['ids'][] = $field_id;
                                                    $this->integrateSubcomponentDataProperties($this->dbdata, $subcomponent_data_properties, $subcomponent_dataloader_class, $module_path_key/*$subcomponent_module_path_key*/);
                                                }
                                            }
                                        }
                                    }

                                    if ($this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]) {
                                        $this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]['ids'] = array_unique($this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]['ids']);
                                        $this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]['data-fields'] = array_unique($this->dbdata[$subcomponent_dataloader_class][$module_path_key/*$subcomponent_module_path_key*/]['data-fields']);
                                    }
                                }
                            }
                        }
